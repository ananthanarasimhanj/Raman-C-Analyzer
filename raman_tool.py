...
# ========= SETTINGS =========
# VERSION is grabbed from the GitHub Tag during the build process
...
def classify_carbon(D, G, TD):
    ...
    # Tuinstra-Koenig Crystallite Size Calculation
    La = (2.4e-10) * (LASER_NM**4) * (1.0 / ID_IG) if (not np.isnan(ID_IG) and ID_IG > 0) else np.nan
    
    ctype = "Unclassified Carbon"
    evidence = []

    # 1. Monolayer Graphene Check
    if TD and I2D_IG > 1.5 and TD["fwhm"] < 40:
        ctype = "Monolayer Graphene"
        evidence = ["High I2D/IG (>1.5)", "Narrow Symmetric 2D Fit"]
    
    # 2. Pristine Graphite Check
    elif TD and I2D_IG < 0.7 and TD["asym"] > 4 and (not D or ID_IG < 0.15):
        ctype = "Pristine Graphite"
        evidence = ["Asymmetric 2D (3D Stacking)", "Negligible D-band"]

    # 3. Multilayer Graphene Check
    elif TD and 0.7 <= I2D_IG <= 1.2:
        ctype = "Multilayer Graphene"
        evidence = ["Intermediate I2D/IG", "Broadened 2D Peak"]

    # 4. Turbostratic Carbon Check
    elif TD and I2D_IG < 0.5 and TD["fwhm"] > 60:
        ctype = "Turbostratic Carbon"
        evidence = ["Very Broad Symmetric 2D", "Rotational Disorder"]

    # 5. Carbon Black / Soot Check
    elif G and G["fwhm"] > 60 and ID_IG > 1.0:
        ctype = "Carbon Black / Soot"
        evidence = ["High ID/IG (>1.0)", "Broad G-band (>60)"]

    # 6. Amorphous Carbon Check
    if G and G["fwhm"] > 85:
        ctype = "Amorphous Carbon"
        evidence = ["Extreme G-Broadening", "Highly Disordered sp2 Clusters"]
...
class RamanApp:
    ...
    def run_analysis(self, filepath):
        ...
        ax_spec.plot(x, y_corr, color="gray", alpha=0.4, label="Baseline Corr.")
        
        colors = {"D": "tab:blue", "G": "tab:green", "2D": "tab:red"}
        for n, f in fits.items():
            if f: ax_spec.plot(f["xr"], f["fit_y"], color=colors[n], lw=2, label=f"{n} Fit")
        
        ax_spec.set_xlabel("Raman shift (cm⁻¹)"); ax_spec.set_ylabel("Intensity (a.u.)")
        ax_spec.set_title(f"Sample: {fname}")
        ax_spec.legend()

        # SIDE LEGEND OUTSIDE GRAPH
        ax_text = fig.add_axes([0.65, 0.12, 0.32, 0.78]); ax_text.axis("off")
        summary = [f"File: {fname}", f"Analyzer: {VERSION}", f"Laser: {LASER_NM} nm", "", "PEAK FITS (Pos / FWHM):"]
        for n in ["D", "G", "2D"]:
            f = fits.get(n)
    ...
    def start(self):
        files = filedialog.askopenfilenames(title="Select Batch Files")
        if not files: return
        res_dir = os.path.dirname(files[0])
        self.log_m(f"--- Starting Analysis v{VERSION} ---")
        for f in files:
            try: p = self.run_analysis(f); self.log_m(f"DONE: {os.path.basename(p)}")
            except Exception as e: self.log_m(f"FAILED: {os.path.basename(f)} | {e}")
        if messagebox.askyesno("Complete", "Batch finished. Open results folder?"):
